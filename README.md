...
* Look at the changes in [this pull request](https://github.com/equalexperts-assignments/equal-experts-mortal-unadulterated-bindle-a046395c25fa/pull/2)
...
* When you have completed the assignment, you need to  push your code to this repository and [mark the assignment as completed by clicking here](https://app.snapcode.review/submission_links/78e9c12e-8552-4a17-8e05-44ea5a1715fb).
...
### Approach
1. File Utilization: Leveraged three provided files: db.py, ingest.py, and outliers.py, for different functionalities within the data pipeline.
2. Database Operations: Utilized db.py primarily for database-specific operations, encapsulating functions related to database connectivity, schema creation, transactions, and data insertion.
3. Ingestion Logic: Implemented batch insertion techniques to optimize performance, minimizing database calls and ensuring that the ingestion process completes within the specified timeframe of 10 seconds. For duplicate handling used Id as a PRIMARY_KEY 
4. Outliers Detection: Employed outliers.py to identify outliers within the dataset using a defined SQL query. The SQL query calculates the count for each week number in a year, computes the average, and applies a predefined formula to detect outliers based on a threshold specified in the provided documentation.
5. SQL Query Explanation: The SQL query within outliers.py calculates the count of votes for each week number in a year, groups the results by year and week number, and computes the average count of votes per week.
It then applies a condition to identify outliers based on a predefined threshold, comparing the count of votes for each week against the average count.
```
    SELECT
    ...
    ORDER BY
        Year, WeekNumber;
```
Finally, the query orders the results by year and week number, providing insights into the distribution of outliers over time.
6. Testing Coverage: Verified that the implemented logic meets the requirements specified in the documentation, maintaining consistency and reliability of the data pipeline.


#### What kind of data quality measures would you apply to your solution in production?
1. Schema Validation: Validate incoming data against predefined schemas to ensure it conforms to the expected structure and data types. 
2. Data Cleansing: Handle missing or inconsistent data. This may include techniques such as imputation, deduplication, and normalization.
3. Data Profiling: Profile the incoming data to understand its characteristics, distributions, and anomalies. This helps in identifying potential data quality issues such as outliers.
4. Duplicate Detection: Implement fuzzy matching.

#### What would need to change for the solution scale to work with a 10TB dataset with 5GB new data arriving each day?
To scale the solution to work with a 10TB dataset with 5GB of new data arriving each day, I would like to consider several aspects, including optimizing the ingestion process, handling large volumes of data efficiently, and ensuring the system's stability and performance. Here are some changes and considerations:

1. Database Optimization: Use a distributed database or a database that can handle large datasets efficiently, such as Apache Spark, or Google BigQuery. These databases are designed to handle petabytes of data and can scale horizontally.
2. Batch Processing: Increase the batch size to handle larger volumes of data efficiently. Instead of processing data line by line, read and process data in larger batches to minimize overhead and improve performance.
3. Parallel Processing: Implement parallel processing techniques to distribute the workload across multiple nodes or cores. This can significantly reduce processing time, especially when dealing with large datasets.
```
    NOTE: In our example since we are calculating outlier on year/weekNumber - I would `partition` my data accordingly to help the processor evaluate them in parallel.
```
4. Data Compression: Compress the data before ingestion to reduce storage and bandwidth requirements. This can help in optimizing storage utilization and speeding up data transfer.

#### Please tell us in your modified README about any assumptions you have made in your solution (below).
Few assumptions that I made is: 
1. I saw in `uncommitted/votes.jsonl` that there are few records with 5 fields and few with 4. I have not dropped `UserId` while ingesting this and placed NULL when field not available.
```
    {"Id":"48838","PostId":"14623","VoteTypeId":"10","CreationDate":"2021-09-03T00:00:00.000"}
    {"Id":"48839","UserId":"30559","PostId":"1237","VoteTypeId":"5","CreationDate":"2021-09-03T00:00:00.000"}
```
2. The filename passed to ingest.py will always be the first command line argument.
```
    if len(sys.argv) < 2:
...
```
3. warehouse, schema, table_name and view_name is constant and hardcoded
